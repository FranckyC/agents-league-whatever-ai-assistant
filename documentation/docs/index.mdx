...
<div className="flex flex-col items-center my-6">
    ...
    <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">Microsoft 365 Copilot/AI Architect - MVP M365 Development/Copilot Extensibility</p>
...
<div className="relative my-10 p-6 rounded-xl border border-purple-200 dark:border-purple-800 bg-gradient-to-br from-purple-50/50 to-cyan-50/50 dark:from-purple-900/20 dark:to-cyan-900/20">
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    <div className="flex flex-col items-center">
      <p className="font-semibold text-sm mb-2">📺 Teams experience</p>
    ...
    <div className="flex flex-col items-center">
      <p className="font-semibold text-sm mb-2">💡 Copilot experience</p>
...
flowchart LR
    subgraph notes1 [" "]
        direction TB
        N1["💡 The router agent outputs a JSON object according to a specific schema so we can filter on properties."]
    end

    subgraph workflow [" "]
        direction TB
        A([🟢 OnConversationStart]) --> B["🔀 Router Agent (router-agent)"]
        B --> C{Query Category}
        C -- "Human Resources" --> D["📋 HR Agent (hr-agent)"]
        C -- "Information Technology" --> E["🖥️ IT Agent (it-agent)"]
        C -- "else" --> F["🛡️ Fallback Agent (fallback-agent)"]
        D --> G([🔴 End Conversation])
        E --> G
        F --> G
    end
    ...
    subgraph notes2 [" "]
        direction TB
        N2["🔧 HR and IT agents have an MCP tool they can use to accomplish their tasks."]
        N3["🚫 Fallback agent to nicely handle every non IT or HR question."]
    end
...
### Secure MCP Server with OAuth Identity Passthrough + claims-based JWT validation

A custom stateless MCP server that leverages the Agent Service [OAuth Identity Passthrough](https://learn.microsoft.com/en-us/azure/ai-foundry/agents/how-to/mcp-authentication?view=foundry#oauth-identity-passthrough) feature to securely execute requests **on behalf of the authenticated user** through MCP tools. Incoming requests from the Agent Service are verified using a claims-based JWT validation (expiration, audience, tenant ID, and issuer).
...
<Tabs>
  <TabItem value="anti-sprawl" label="🧩 Anti-Agent Sprawl" default>
...
Because it is an Azure resource like any other, it also means all the infrastructure provisioning can be automated using Infrastructure as Code (IaC) tools like Bicep or Terraform, which is essential for maintaining consistency across different environments (development, testing, production).
  ...
  <TabItem value="copilot" label="💡 Maximize Copilot Investment">
...
| **Cost** | ✅ Already included with Copilot licenses | ❌ Additional infrastructure and maintenance costs |
| **Reusability** | ✅ Same connectors work across custom engine agents, Copilot Studio, and Agent Builder | ❌ Tied to your specific implementation |
| **Maintenance** | ✅ Microsoft handles scaling, updates, and optimizations | ❌ You own the full lifecycle |
...
<div className="border border-purple-200 dark:border-purple-800 rounded-lg p-5">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div className="bg-purple-50 dark:bg-purple-900/20 rounded-md p-3">
            <p className="font-medium text-sm mb-1">🐛 Debug card</p>
            <p className="text-xs mb-0">For developers to inspect agent reasoning and steps taken</p>
        </div>
        <div className="bg-purple-50 dark:bg-purple-900/20 rounded-md p-3">
            <p className="font-medium text-sm mb-1">🎫 Ticket submission form</p>
            <p className="text-xs mb-0">To collect structured issue details</p>
        </div>
        <div className="bg-purple-50 dark:bg-purple-900/20 rounded-md p-3">
            <p className="font-medium text-sm mb-1">⚠️ Disclaimer card</p>
            <p className="text-xs mb-0">Displayed on first interaction or new chat sessions</p>
        </div>
        <div className="bg-purple-50 dark:bg-purple-900/20 rounded-md p-3">
            <p className="font-medium text-sm mb-1">🔐 MCP tool approval card</p>
