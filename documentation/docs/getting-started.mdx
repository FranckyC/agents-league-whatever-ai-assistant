...
<div className="text-center my-6">
  <p className="text-lg text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-cyan-500 font-medium">
    Follow the steps below to deploy the full solution — from SharePoint data preparation all the way to a working Teams app.
  </p>
</div>

This guide walks you through **five stages**:

> **① [Prerequisites](#prerequisites)** → **② [Prepare the data](#prepare-the-data)** → **③ [Deploy to Azure](#deploy-infrastructure-in-azure)** → **④ [Configure MCP & agents](#configure-mcp-in-foundry)** → **⑤ [Ship the Teams app](#package-and-upload-the-teams-app)**
...
| **Bicep** | v0.38.3+ — [Install guide](https://learn.microsoft.com/en-us/azure/azure-resource-manager/bicep/install#install-manually) |
...
<Tabs>
  <TabItem value="hr" label="📋 HR site" default>
...
<div className='flex space-x-4 justify-center items-center my-4'>
  ...
  <TabItem value="it" label="🖥️ IT site">
...
3. Create a new blank list called **Tickets** and add the following columns. You'll need to set the exact same column **internal names** for the MCP tool configuration later. This list is used to create tickets submitted by the user from Teams via the MCP tool `submit_ticket`.
        ...
        | `ticketSeverity` | Choice — values: `Critical`, `Medium`, `Low` |
...
<details>
<summary>✅ <strong>Succeeded</strong> deployment stack with all resources created</summary>
...
<details>
<summary>✅ All resources created in your resource group</summary>
...
<details>
<summary>✅ Environment variables linked to Key Vault</summary>
...
<details>
<summary>✅ Application code deployed successfully</summary>
...
<details>
<summary>✅ Server started in the application log stream</summary>
...
2. Add the following **delegated** permission for Microsoft Graph API:
    - `Sites.ReadWrite.All` — Allows the agent to read and write data in SharePoint on behalf of the user (i.e., Copilot Retrieval API and SharePoint list items for ticket submission).
...
1. Go to the Microsoft Foundry portal and open your project from [https://ai.azure.com/nextgen](https://ai.azure.com/nextgen). Then navigate to _Build_ → _Tools_ and click _Connect a tool_.
    ...
    <div className='flex flex-col md:flex-row gap-6 items-start my-4'>
        <div className='flex-1'>
            <ul className='list-none pl-0 space-y-4 text-sm mb-4'>
                <li><strong>Name</strong> — <code>mcp</code></li>
                <li><strong>Remote MCP Server endpoint</strong><br/><code>https://&lt;your-app-service-name&gt;.azurewebsites.net/api/mcp</code></li>
                <li><strong>Authentication</strong> — <em>OAuth Identity Passthrough</em></li>
            </ul>

            <p className='text-sm mt-5 mb-3 font-medium'>Then fill in the OAuth details:</p>

            <ul className='list-none pl-0 space-y-4 text-sm mb-4'>
                <li><strong>Client ID</strong> — The client ID of the Entra ID application you created above</li>
                <li><strong>Client secret</strong> — The client secret you created above</li>
                ...
                <li><strong>Scope</strong> — <code>Sites.ReadWrite.All</code></li>
            </ul>
...
<div className='flex flex-col md:flex-row gap-6 items-start my-6'>
    <div className='flex-1'>
        <div className="border border-purple-200 dark:border-purple-800 rounded-lg p-4">
            <p className="font-semibold text-lg mb-1">⚙️ <code>workflow-agent</code></p>
...
2. Open the Microsoft Teams client (web or desktop) and go to _Apps_ → _Manage your apps_ → _Upload an app_ → _Upload a customized app_. Select the `appPackage.dev.zip` file to sideload the bot to Teams.

    <div className='flex justify-center items-center my-4'>
        <img className="w-[80%] rounded-lg shadow-lg" src={AddTeamsApp} />
    </div>

3. The app should now be available from both the **Teams** and **Copilot** channels, ready to use with the starter prompts. 🎉
...
Now the fun part: testing the agent in Copilot! 🎉

---

### First Interaction — Disclaimer & Authentication
...
### Ticket Submission — Write Operations

Test the ticket submission system by asking a question that would trigger the IT agent, for example: _"I have an issue with my laptop, I want to raise a ticket"_. The IT agent should recognize that this is an IT-related question and direct it to the IT agent. The IT agent will then ask for more details and send a card to gather information:
...
### Debug Mode — Inspect Agent Reasoning
...
## Bonus — Tool Approval with Prompt Agents
...
Change the app service environment variable `ENV_AZURE_DEPLOY_WORKFLOW_AGENT` to target the `hr-agent` directly:
...
Update the MCP tools configuration to ask for approval:
...
To avoid an approval request endless loop, update the agent to let it select the tool when needed:

<div className='flex justify-center items-center my-6'>
    <img className="w-[40%] rounded-lg shadow-lg" src={HrAgentToolSelection} />
</div>

Now the next question you ask (e.g., _"What is the parental leave policy?"_), you should be prompted to approve the tool calls in the portal before getting the answer:
